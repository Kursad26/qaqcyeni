/*
  # Aktivite ID Ekleme - NOI Modülü

  ## Değişiklikler
  1. noi_requests tablosuna activity_id kolonu ekleme
  2. create_noi_request fonksiyonunu güncelleme (activity_id parametresi ekleme)
  3. Index ekleme
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'noi_requests' AND column_name = 'activity_id'
  ) THEN
    ALTER TABLE noi_requests
    ADD COLUMN activity_id uuid REFERENCES project_activities(id) ON DELETE SET NULL;

    -- Index ekle
    CREATE INDEX idx_noi_requests_activity_id ON noi_requests(activity_id);

    -- Comment ekle
    COMMENT ON COLUMN noi_requests.activity_id IS
    'Foreign key to activity. Defines which activity this NOI request is for.';
  END IF;
END $$;
...
COMMENT ON FUNCTION create_noi_request IS
'Creates a new NOI request with automatic numbering. Includes manufacturing_unit_id and activity_id for hierarchical structure.';
