-- Migration: Add manufacturing_unit_id to noi_requests table (v3 - CASCADE fix)
-- Date: 2025-11-22
...
-- 2. Drop ALL versions of the function (CASCADE)
DROP FUNCTION IF EXISTS create_noi_request CASCADE;
...
BEGIN
  ...
  SELECT * INTO v_settings FROM noi_settings WHERE project_id = p_project_id;

  IF v_settings IS NULL THEN
    INSERT INTO noi_settings (project_id, number_prefix, current_number)
    VALUES (p_project_id, 'NOI', 0) RETURNING * INTO v_settings;
  END IF;

  v_new_number := v_settings.current_number + 1;
  v_noi_number := v_settings.number_prefix || '-' || LPAD(v_new_number::TEXT, 4, '0');

  UPDATE noi_settings SET current_number = v_new_number, updated_at = NOW()
  WHERE project_id = p_project_id;

  INSERT INTO noi_requests (
    project_id, noi_number, status, date, time, company_id,
    const_personnel_id, qc_personnel_id, location, hold_point_id,
    manufacturing_unit_id, revision_number, original_noi_number, created_by
  ) VALUES (
    p_project_id, v_noi_number, 'pending_approval', p_date, p_time, p_company_id,
    p_const_personnel_id, p_qc_personnel_id, p_location, p_hold_point_id,
    p_manufacturing_unit_id, 0, v_noi_number, v_user_id
