...
export async function uploadImageToCloudinary(file: File, folderName: string = 'saha-egitimleri'): Promise<string> {
  try {
...
export async function uploadDocumentToCloudinary(file: File, folderName: string = 'egitim-dokumanlari'): Promise<string> {
  try {
    const cloudName = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME;
    const apiKey = import.meta.env.VITE_CLOUDINARY_API_KEY;
    const apiSecret = import.meta.env.VITE_CLOUDINARY_API_SECRET;
...
export async function uploadFileToCloudinary(file: File, folderName?: string): Promise<string> {
  // Dosya tipine göre uygun fonksiyonu çağır
  if (file.type.startsWith('image/')) {
    return uploadImageToCloudinary(file, folderName);
  } else {
    return uploadDocumentToCloudinary(file, folderName);
  }
}
