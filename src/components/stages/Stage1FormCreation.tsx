...
interface Personnel {
  ...
  last_name: string;
  user_id?: string;
  field_observation_access?: boolean;
  role_id?: string;
...
export function Stage1FormCreation({ formData, onChange, onSubmit, disabled = false }: Stage1Props) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [
        ...
        { data: unitsData },
        { data: activitiesData },
        { data: allowedRolesData }
      ] = await Promise.all([
        supabase.from('companies').select('id, name').eq('project_id', currentProject.id).eq('is_active', true).order('name'),
        supabase.from('personnel').select('id, first_name, last_name, user_id, field_observation_access, role_id, user_profiles(full_name)').eq('project_id', currentProject.id).order('first_name'),
        ...
        supabase.from('project_activities').select('id, name, manufacturing_unit_id').eq('project_id', currentProject.id).eq('is_active', true).order('name'),
        supabase.from('field_observation_allowed_roles').select('role_id').eq('project_id', currentProject.id).eq('is_active', true)
      ]);

      // Get list of allowed role IDs
      const allowedRoleIds = allowedRolesData?.map(ar => ar.role_id) || [];

      // Filter personnel based on field_observation_access and allowed roles
      let finalPersonnelList = personnelData || [];

      // Only filter if there are allowed roles configured
      if (allowedRoleIds.length > 0) {
        finalPersonnelList = finalPersonnelList.filter(p =>
          p.field_observation_access === true &&
          p.role_id &&
          allowedRoleIds.includes(p.role_id)
        );
      } else {
        // If no allowed roles configured, show all personnel with field_observation_access
        finalPersonnelList = finalPersonnelList.filter(p =>
          p.field_observation_access === true
        );
      }
  ...
  const getActivityOptions = (manufacturingUnitId: string | undefined) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <SearchableDropdown
          ...
          onChange={(value) => onChange({ manufacturing_unit_id: value, activity_id: undefined })}
        ...
        <SearchableDropdown
          ...
          placeholder={formData.manufacturing_unit_id ? "Aktivite seçiniz" : "Önce imalat birimi seçin"}
          required
