...
import {
  ...
  Edit2, Trash2, X, Filter, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export function NoiPage() {
  ...
  const [allNoisCurrentPage, setAllNoisCurrentPage] = useState(1);
  ...
  // Re-filter pending NOIs when isApprover or noiRequests changes
  useEffect(() => {
    if (noiRequests.length > 0) {
      filterPendingNois(noiRequests);
    }
  }, [isApprover, isAdmin, noiRequests]);
  ...
  const exportToExcel = () => {
    ...
    const data = filteredNois.map(noi => ({
      ...
      'Mahal': noi.location || '-',
      'Aktivite - Hold Point': noi.activity?.name && noi.hold_point?.name
        ? `${noi.activity.name} - ${noi.hold_point.name}`
        : (noi.hold_point?.name || '-'),
  ...
  const pendingColumns: ColumnDefinition[] = [
    ...
    {
      key: 'hold_point',
      header: 'Aktivite - Hold Point',
      width: 220,
      render: (row: NoiRequestWithDetails) =>
        row.activity?.name && row.hold_point?.name
          ? `${row.activity.name} - ${row.hold_point.name}`
          : (row.hold_point?.name || '-')
    },
    {
    ...
    },
    {
      ...
      render: (row: NoiRequestWithDetails) => {
        ...
        // For admins and approvers - rejected (also show cancel button)
        if (row.status === 'rejected' && (isAdmin || isApprover)) {
          return (
            <div className="flex gap-2">
              <button
                onClick={() => handleCancel(row.id)}
                ...
                <div className="p-4">
                  <div className="mb-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                      ...
                      {/* Hold Point Filter */}
                      <SearchableFilterGroup
                        label="Aktivite - Hold Point"
                        options={Array.from(new Set(pendingNois.filter(n => n.hold_point_id).map(n => n.hold_point_id)))
                          .map(holdPointId => {
                            const noi = pendingNois.find(n => n.hold_point_id === holdPointId);
                            const holdPoint = noi?.hold_point;
                            const activity = noi?.activity;
                            if (!holdPoint) return null;
                            const label = activity?.name && holdPoint.name
                              ? `${activity.name} - ${holdPoint.name}`
                              : holdPoint.name;
                            return { value: holdPointId as string, label };
                          })
                          .filter((opt): opt is { value: string; label: string } => opt !== null)}
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {pendingFilters.holdPoint.length > 0 && (
                          <button
                            ...
                            className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm flex items-center gap-1 hover:bg-blue-200 transition"
                          >
                            Aktivite - Hold Point ({pendingFilters.holdPoint.length})
                            <X className="w-3 h-3" />
                          </button>
                        )}
                        {pendingStartDate && (
                  ...
                  {/* Pagination for Pending NOIs */}
                  {sortedPendingNois.length > ITEMS_PER_PAGE && (() => {
                    const totalPages = Math.ceil(sortedPendingNois.length / ITEMS_PER_PAGE);
                    return (
                      <div className="mt-6 flex items-center justify-between">
                        <div className="text-sm text-gray-600">
                          Toplam {sortedPendingNois.length} kayıt - Sayfa {pendingCurrentPage} / {totalPages}
                        </div>
                          ...
                          <div className="flex items-center space-x-2">
                            {[...Array(Math.min(5, totalPages))].map((_, i) => {
                              ...
                              if (totalPages <= 5) {
                                ...
                                pageNum = pendingCurrentPage - 2 + i;
                              }
                              ...
                              return (
                                <button
                                  key={i}
                                  onClick={() => {
                                    setPendingCurrentPage(pageNum);
                                    window.scrollTo({ top: 0, behavior: 'smooth' });
                                  }}
                                  className={`px-3 py-2 border rounded-lg ${
                                    pendingCurrentPage === pageNum
              ...
              <div className="p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                      ...
                      <SearchableFilterGroup
                        ...
                        options={Array.from(new Set(noiRequests.filter(n => n.hold_point_id).map(n => n.hold_point_id)))
                          .map(holdPointId => {
                            const noi = noiRequests.find(n => n.hold_point_id === holdPointId);
                            const holdPoint = noi?.hold_point;
                            const activity = noi?.activity;
                            if (!holdPoint) return null;
                            const label = activity?.name && holdPoint.name
                ...
                {groupedNois ? (
                  ...
                  <ResizableTable
                    ...
                    data={sortedNois.slice(
                      (allNoisCurrentPage - 1) * ITEMS_PER_PAGE,
                      allNoisCurrentPage * ITEMS_PER_PAGE
                    )}
                ...
                {/* Pagination for All NOIs */}
                {sortedNois.length > ITEMS_PER_PAGE && (() => {
                  const totalPages = Math.ceil(sortedNois.length / ITEMS_PER_PAGE);
                  return (
                    <div className="mt-6 flex items-center justify-between">
                      <div className="text-sm text-gray-600">
                        Toplam {sortedNois.length} kayıt - Sayfa {allNoisCurrentPage} / {totalPages}
                      </div>
                      <div className="flex items-center space-x-2">
                        <button
                          onClick={() => {
                            setAllNoisCurrentPage(allNoisCurrentPage - 1);
                            window.scrollTo({ top: 0, behavior: 'smooth' });
                          }}
                          disabled={allNoisCurrentPage === 1}
                          className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1"
                        >
                          <ChevronLeft className="w-4 h-4" />
                          <span>Önceki</span>
                        ...
                        <div className="flex items-center space-x-2">
                          {[...Array(Math.min(5, totalPages))].map((_, i) => {
                            ...
                            if (totalPages <= 5) {
                              pageNum = i + 1;
                            } else if (allNoisCurrentPage <= 3) {
                              pageNum = i + 1;
                            } else if (allNoisCurrentPage >= totalPages - 2) {
                              pageNum = totalPages - 4 + i;
                            } else {
                              pageNum = allNoisCurrentPage - 2 + i;
                            }
                            ...
                            return (
                              <button
                                key={i}
                                onClick={() => {
                                  setAllNoisCurrentPage(pageNum);
                                  window.scrollTo({ top: 0, behavior: 'smooth' });
                                }}
                                className={`px-3 py-2 border rounded-lg ${
                                  allNoisCurrentPage === pageNum
                                    ? 'bg-blue-600 text-white border-blue-600'
                                    : 'border-gray-300 hover:bg-gray-50'
                                }`}
                              >
                        ...
                        <button
                          ...
                          disabled={allNoisCurrentPage === totalPages}
                          className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1"
                        >
                          <span>Sonraki</span>
                          <ChevronRight className="w-4 h-4" />
