...
export function FieldTrainingPage() {
  ...
  const generatePDF = async (report: FieldTrainingReport) => {
    try {
      ...
      const templateId = import.meta.env.VITE_CARBONE_TEMPLATE_FIELD_TRAINING;
  ...
  const filteredReports = reports.filter(report => {
    ...
    const matchesTrainingType = filters.trainingType.length === 0 ||
      filters.trainingType.includes(report.training_type);

    // Date range filter
    const reportDate = new Date(report.deadline_date || report.delivery_date || report.created_at);
    ...
    return matchesSearch && matchesStatus && matchesCompany && matchesUnit && matchesOrganizer && matchesTrainingType && matchesStartDate && matchesEndDate;
  });
            ...
            {showFilters && (
              <div className="mb-6 p-4 bg-gray-50 rounded-lg space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-2">Eğitim Türü</label>
                    <div className="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-2 bg-white">
                      {[
                        { value: 'internal', label: 'İç Eğitim' },
                        { value: 'external', label: 'Dış Eğitim' }
                      ].map(option => (
                        <label key={option.value} className="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                          <input
                            type="checkbox"
                            checked={filters.trainingType.includes(option.value)}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setFilters({ ...filters, trainingType: [...filters.trainingType, option.value] });
                              } else {
                                setFilters({ ...filters, trainingType: filters.trainingType.filter(t => t !== option.value) });
                              }
                            }}
                            className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                          />
