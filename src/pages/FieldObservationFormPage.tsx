...
export function FieldObservationFormPage({ reportId, isSidePanel = false, onClose }: FieldObservationFormPageProps) {
  ...
  const handleStage5Approve = async () => {
    ...
    setSaving(true);
    try {
      // Sadece tarihleri karşılaştır (yıl, ay, gün) - saat ve timezone sorunlarını önler
      const plannedDate = new Date(formData.planned_close_date);
      const closingDate = new Date(formData.closing_date || new Date());

      // Sadece tarih kısmını al (yıl, ay, gün)
      const plannedDateOnly = new Date(
      ...
      const newStatus = closingDateOnly <= plannedDateOnly ? 'closed_on_time' : 'closed_late';

      // Debug log - konsolda kontrol edebilmek için
      console.log('Tarih Karşılaştırma:', {
        plannedCloseDate: formData.planned_close_date,
        closingDate: formData.closing_date,
        plannedDateOnly: plannedDateOnly.toLocaleDateString('tr-TR'),
        closingDateOnly: closingDateOnly.toLocaleDateString('tr-TR'),
        isOnTime: closingDateOnly <= plannedDateOnly,
        status: newStatus
      });
