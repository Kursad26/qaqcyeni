...
import { Users, Building2, UserCog, Plus, ToggleLeft, ToggleRight, Settings, UserCircle, Edit2, Save, X, Trash2, Download, Search } from 'lucide-react';
...
export function AdminProjectDetailPage({ projectId }: ProjectDetailPageProps) {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'companies' | 'personnel' | 'settings' | 'roles'>('users');
  ...
  const [personnelSortColumn, setPersonnelSortColumn] = useState<string>('');
  ...
  const togglePersonnelNoiCreate = async (personnelId: string, currentAccess: boolean) => {
    const { error } = await supabase
      .from('personnel')
      .update({ noi_create: !currentAccess })
      .eq('id', personnelId);

    if (error) {
      alert('NOI oluşturma yetkisi güncellenirken hata oluştu');
    } else {
      await fetchProjectData();
    }
  };
  ...
  const handlePersonnelSort = (column: string) => {
    if (personnelSortColumn === column) {
  ...
  const sortedPersonnel = [...filteredPersonnel].sort((a: any, b: any) => {
    if (!personnelSortColumn) return 0;

    let aValue: any;
    let bValue: any;

    switch (personnelSortColumn) {
      case 'user':
        aValue = a.user_profiles?.full_name || `${a.first_name} ${a.last_name}`;
        bValue = b.user_profiles?.full_name || `${b.first_name} ${b.last_name}`;
        break;
      case 'position':
        aValue = a.position || '';
        bValue = b.position || '';
      ...
      case 'company':
        ...
        break;
      case 'role':
        aValue = a.project_roles?.name || '';
        bValue = b.project_roles?.name || '';
        break;
      default:
        return 0;
    }

    if (aValue < bValue) return personnelSortDirection === 'asc' ? -1 : 1;
    if (aValue > bValue) return personnelSortDirection === 'asc' ? 1 : -1;
    return 0;
  });
  ...
  const exportPersonnelToExcel = () => {
    const data = filteredPersonnel.map((person: any) => ({
      ...
      'NOI Oluşturma': person.noi_create ? 'Aktif' : 'Pasif',
                    ...
                    <thead>
                      <tr className="border-b border-gray-200">
                        <th className="text-left py-3 px-4 font-semibold text-gray-700">
                          <button
                            onClick={() => handlePersonnelSort('user')}
                            className="flex items-center space-x-1 hover:text-blue-600"
                          >
                            <span>Kullanıcı</span>
                            {personnelSortColumn === 'user' && (
                              <span className="text-xs">{personnelSortDirection === 'asc' ? '↑' : '↓'}</span>
                            )}
                          </button>
                        </th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-700">
                          <button
                            onClick={() => handlePersonnelSort('position')}
                            className="flex items-center space-x-1 hover:text-blue-600"
                          >
                            <span>Pozisyon</span>
                            {personnelSortColumn === 'position' && (
                              <span className="text-xs">{personnelSortDirection === 'asc' ? '↑' : '↓'}</span>
                            )}
                          </button>
                        </th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-700">
                          <button
                            onClick={() => handlePersonnelSort('phone')}
                            className="flex items-center space-x-1 hover:text-blue-600"
                          >
                            <span>Telefon</span>
                            {personnelSortColumn === 'phone' && (
                              <span className="text-xs">{personnelSortDirection === 'asc' ? '↑' : '↓'}</span>
                            )}
                          </button>
                        </th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-700">
                          <button
                            onClick={() => handlePersonnelSort('company')}
                            className="flex items-center space-x-1 hover:text-blue-600"
                          >
                            <span>Firma</span>
                            {personnelSortColumn === 'company' && (
                              <span className="text-xs">{personnelSortDirection === 'asc' ? '↑' : '↓'}</span>
                            )}
                          </button>
                        </th>
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-700">
                          <button
                            onClick={() => handlePersonnelSort('role')}
                            className="flex items-center space-x-1 hover:text-blue-600"
                          >
                            <span>Rol</span>
                            {personnelSortColumn === 'role' && (
                              <span className="text-xs">{personnelSortDirection === 'asc' ? '↑' : '↓'}</span>
                            )}
                          </button>
                        </th>
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-gray-700">NOI Oluştur</th>
                    ...
                    <tbody>
                      {sortedPersonnel.map((person: any) => (
                        <tr key={person.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                          ...
                          <td className="py-4 px-4">
                            <button
                              onClick={() => togglePersonnelNoiCreate(person.id, person.noi_create || false)}
                              className="p-1.5 hover:bg-gray-100 rounded-lg transition"
                              title={person.noi_create ? 'NOI Oluşturma Yetkisini Kapat' : 'NOI Oluşturma Yetkisini Aç'}
                            >
                              {person.noi_create ? (
                                <ToggleRight className="w-6 h-6 text-green-600" />
                              ) : (
